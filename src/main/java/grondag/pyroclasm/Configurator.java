...
public class Configurator {
	...
	public static class Volcano {
		...
		private static void recalcBlocks() {
			final Registry<Block> reg = Registry.BLOCK;

			blocksDestroyedByLava.clear();
			for (final String s : SUBSTANCES.blocksDestroyedByVolcanicLava) {
				final Identifier rl = new Identifier(s);
				if (reg.containsId(rl)) {
					final Block b = reg.get(rl);
			...
			for (final String s : SUBSTANCES.blocksSafeFromVolcanicLava) {
				final Identifier rl = new Identifier(s);
				if (reg.containsId(rl)) {
					final Block b = reg.get(rl);

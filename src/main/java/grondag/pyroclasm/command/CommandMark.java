...
public class CommandMark  {
	...
	public void execute(MinecraftServer server, ServerPlayerEntity player, String[] args) {
		try {
			...
			final Map<BlockPos, VolcanoNode> near = vm.nearbyVolcanos(player.getBlockPos());
			final long[] data = new long[near.size()];
			int i = 0;
			for (final Map.Entry<BlockPos, VolcanoNode> entry : near.entrySet()) {

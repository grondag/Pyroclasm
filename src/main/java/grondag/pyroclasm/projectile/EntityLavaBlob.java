...
public class EntityLavaBlob extends Entity {
	...
	private void land() {
		if (world.isClient) {
			world.playSound(x, y, z, ModSounds.bomb_impact, SoundCategory.AMBIENT, 8.0F + random.nextFloat() * 2.0F,
				0.8F + random.nextFloat() * 0.2F, false);
			for (int i = 0; i < 20; i++) {
				world.addParticle(ParticleTypes.LAVA, x - 1 + random.nextFloat() * 2, y + 1.0, z - 1 + random.nextFloat() * 2, 0.0D,
					0.0D, 0.0D);
			}
		} else {
	...
	public void remove() {
		if (!world.isClient) {
			if (Configurator.DEBUG.enableLavaBombTrace) {
				Pyroclasm.LOG.info("Lava bomb %d dead @ %f, %f, %f", id, x, y, z);
			}
	...
	private void destroyCollidingDisplaceableBlocks(Box bb) {
		...
		final BlockPos.PooledMutable blockpos$pooledmutableblockpos = BlockPos.PooledMutable.get();

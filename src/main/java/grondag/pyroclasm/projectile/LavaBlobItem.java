...
public class LavaBlobItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World worldIn, PlayerEntity playerIn, Hand hand) {
        ...
        worldIn.playSound((PlayerEntity) null, playerIn.getX(), playerIn.getY(), playerIn.getZ(), SoundEvents.ENTITY_SNOWBALL_THROW, SoundCategory.NEUTRAL, 0.5F,
                0.4F / (RANDOM.nextFloat() * 0.4F + 0.8F));

        if (!worldIn.isClient) {
            final EntityLavaBlob blob = new EntityLavaBlob(worldIn, LavaSimulator.FLUID_UNITS_PER_BLOCK,
                    new Vec3d(playerIn.getX(), playerIn.getY() + playerIn.getStandingEyeHeight() - 0.10000000149011612D, playerIn.getZ()), Vec3d.ZERO);

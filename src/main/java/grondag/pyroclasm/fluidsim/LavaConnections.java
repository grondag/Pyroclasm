...
public class LavaConnections extends AbstractLavaConnections {
	...
	private final ArrayMappingConsumer<CellChunk, Flowable> chunkConsumer = new ArrayMappingConsumer<CellChunk, Flowable>(
		(CellChunk c, Consumer<Flowable> r) -> {
			...
			c.forEach(cell -> {
				...
				final Flowable keeper = cell.getFlowChain();
				if (keeper != null) {
					r.accept(keeper);
				}
			});
	...
	public final void doCellSetup() {
		final int chunkCount = sim.cells.chunkCount();
		if (chunkCount == 0)
			return;

		setupCounter.startRun();
		Simulator.SCATTER_GATHER_POOL.completeTask(sim.cells.rawChunks(), ScatterGatherThreadPool.POOL_SIZE, chunkConsumer);
		setupCounter.endRun();
		setupCounter.addCount(chunkCount);
	}
	...
	protected final void doStepInner() {
		...
		Flowable[] inputs = toProcess.toArray();

		if (inputs.length < Configurator.PERFORMANCE.concurrencyThreshold / 2) {
			stepCounter.startRun();
			stepCounter.addCount(processStepToCompletion(inputs, stepIndex, f -> f.doStep()));
			stepCounter.endRun();
		} else {
			parallelStepCounter.startRun();

			do {
				parallelStepCounter.addCount(inputs.length);
				Simulator.SCATTER_GATHER_POOL.completeTask(flowTask.prepare(inputs, stepIndex));
				if (flowTask.isOutputEmpty()) {
					break;
				}
				inputs = flowTask.outputs();
			} while (true);

			parallelStepCounter.endRun();
		}
	}
	...
	protected class FlowTask implements ISharableTask {
		...
		protected final FlowTask prepare(Flowable[] inputs, int step) {
			...
			endIndex = size;
			this.step = step;
			batchSize = Math.max(1, (size + BATCH_COUNT - 1) / BATCH_COUNT);
			outputs.clear();
			return this;
		}

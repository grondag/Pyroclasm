...
public abstract class AbstractLavaCell {
	...
	/**
	 * True if this cell is new or has expanded. Used to determine if
	 * updateConnectionsIfNeeded should do anything.
	 */
	private boolean isConnectionUpdateNeeded = true;

	public final void invalidateConnections() {
		if (!isConnectionUpdateNeeded) {
			isConnectionUpdateNeeded = true;
		}
	}

	public final boolean isConnectionUpdateNeeded() {
		return isConnectionUpdateNeeded;
	}

	public final void clearConnectionUpdate() {
		if (isConnectionUpdateNeeded) {
			isConnectionUpdateNeeded = false;
		}
	}
	...
	/**
	 * Floor as fluid units instead of levels - derived from floor each time floor
	 * is set. Here to avoid multiplying floor each connection pass.
	 */
	private int floorUnits;

	/**
	 * Floor of cell as Y level instead of levels - derived from floor each time
	 * floor is set. Is the lowest level of cell that can contain lava.
	 * <p>
	 *
	 * If the change is caused by lava cooling, will not affect the retention level
	 * of neighboring cells that still contain lava.
	 */
	private short floorY;

	/** See {@link #floorLevel()} */
	public final void setFloorLevel(int newFloorLevel, boolean isFlowFloor) {
		if (newFloorLevel != floorLevel || isFlowFloor != isBottomFlow) {
			invalidateConnections();

			floorLevel = newFloorLevel;
			floorUnits = newFloorLevel * LavaSimulator.FLUID_UNITS_PER_LEVEL;
			isBottomFlow = isFlowFloor;
			floorY = (short) getYFromFloor(floorLevel);
	...
	/**
	 * First (lowest) fluid level that can contain fluid, EXCLUSIVE. Values range
	 * from 0 to (256 * LEVELS_PER_BLOCK) - 1 Levels in Y=0 for example, are 0 thru
	 * 11. ALWAYS USE setFloor() instead of floor to maintain bottomY.
	 */
	...
	/**
	 * Y of the block that forms the floor of this cell. Will be same as
	 * {@link #floorY()} unless the floor is at a block boundary. In that case, the
	 * block Y will be one below.
	 */
	public final int floorBlockY() {
		return floorFlowHeight() == 0 ? floorY() - 1 : floorY();
	}

	/**
	 * Flow height of solid portion of block at {@link #floorY()} Will be 0 if floor
	 * is not a flow block. Will also be 0 if floor is a full-height flow block at
	 * Y-1. Will be in range 1-11 if floor is within the block at Y. Note that 12 is
	 * not a valid result because that would mean block at Y is not the floor
	 * because it could not contain lava.
	 */
	public final int floorFlowHeight() {
		...
		// 23 -> 11
		// 24 -> 0
		return floorLevel % TerrainState.BLOCK_LEVELS_INT;
	}

	/**
	 * True if the solid block under this columns is a flow block. Note that it must
	 * be true if bottomFlowHeight > 0;
	 */
	public final boolean isBottomFlow() {
		return isBottomFlow;
	}
	...
	public final void setCeilingLevel(int newCeilingLevel) {
		if (newCeilingLevel != ceilingLevel) {
			invalidateConnections();
			ceilingLevel = newCeilingLevel;
			ceilingUnits = newCeilingLevel * LavaSimulator.FLUID_UNITS_PER_LEVEL;
			ceilingY = (short) getYFromCeilingLevel(ceilingLevel);
		}
	}

	/**
	 * Last (highest) block level that can contain fluid, INCLUSIVE. Values range
	 * from 1 to (256 * LEVELS_PER_BLOCK) Levels in Y=0, for example are 1 thru 12.
	 * ALWAYS USE setCeiling() to maintain topY.
	 */
	public final int ceilingLevel() {
		return ceilingLevel;
	}

	/* Ceiling as fluid units */
	public final int ceilingUnits() {
		return ceilingUnits;
	}
	...
	public final boolean isOpenToSky() {
		return ceilingY == 255;
	}
	...
	public final int volumeUnits() {
		return ceilingUnits - floorUnits;
	}
	...
	public final void setFluidUnits(int newUnits) {
		if (newUnits < 0) {
			assert false : String.format("Negative fluid units detected.  NewAmount=%d cell ID=%s", newUnits, hashCode());
		newUnits = 0;
		}
		fluidUnits = newUnits;
	}
	...
	/**
	 * Called when the floor of this cell is changed to force recalc of anything in
	 * this cells or neighbor cells that depend on the floor.
	 */
	...
	/**
	 * Absolute surface level as measured in fluid units. Solid blocks/levels below
	 * count as if they contained fluid. Does not reflect excess fluid for cells
	 * under pressure. If cell has no fluid, is the level of the cell floor, as if
	 * it were fluid.
	 */
	public final int worldSurfaceUnits() {
		return Math.min(ceilingUnits(), floorUnits() + fluidUnits());
	}

	/**
	 * Top level that contains fluid in the world. For columns under pressure does
	 * not reflect extra fluid - is limited by the cell ceiling. Will return cell
	 * floor if there is no fluid in the cell. If cell has any fluid at all, will be
	 * at least one level above floor.
	 */
	public final int worldSurfaceLevel() {
		// fluidLevels() handles case of showing level = 1 when fluid units are less
		// than a full level
		return Math.min(ceilingLevel(), floorLevel() + fluidLevels());
	}

	/**
	 * Returns the world Y-level of the uppermost block containing lava. Returns
	 * {@link #floorY()} if the cell does not contain fluid.
	 */
	public final int worldSurfaceY() {
		if (isEmpty())
			return floorY();

		return getYFromCeilingLevel(worldSurfaceLevel());
	}
	...
	public final int worldSurfaceFlowHeight() {
		...
		// 24 -> 12
		// 25 -> 1
		return worldSurfaceLevel() - worldSurfaceY() * TerrainState.BLOCK_LEVELS_INT;
	}
	...
	public final int pressureSurfaceUnits() {
		final int floorUnits = floorUnits();
		final int volumeUnits = ceilingUnits() - floorUnits;
		final int fluidUnits = fluidUnits();

		return pressureSurface(floorUnits, volumeUnits, fluidUnits);
	}

	public final int pressureSurfaceLevel() {
		final int units = pressureSurfaceUnits();
		return units == 0 ? 0 : Math.max(1, units / LavaSimulator.FLUID_UNITS_PER_LEVEL);
	}

	public static final int pressureSurface(int floorUnitsIn, int volumeUnitsIn, int fluidUnitsIn) {
		return fluidUnitsIn > volumeUnitsIn ? floorUnitsIn + volumeUnitsIn + (fluidUnitsIn - volumeUnitsIn) * PRESSURE_FACTOR : floorUnitsIn + fluidUnitsIn;
	}
	 ...
	 *
	 * 10 Sa >= Sb assume ceiling of cell A is higher than B 11 Fb + Ub + cXb - Fa -
	 * Ua = 0 find max value for Xb such that Xa = 0 12 t = Ua + Ub + Xb Ua and Ub
	 * are fixed at max depth, t and Xb are unknown 13 Xb = t - Ua - Ub rearrange
	 * #12 14 Fb + Ub + c(t - Ua - Ub) - Fa - Ua = 0 substitute #13 into #11 ct -
	 * cUa - cUb = Fa + Ua - Fb - Ub solve for t ct = Fa + Ua - Fb - Ub + cUa + cUb
	 * ct = Fa + Ua + cUa - Fb - Ub + cUb ct = Fa + (c+1)Ua - Fb + (c-1)Ub t = (Fa +
	 * (c+1)Ua - Fb + (c-1)Ub) / c when total in both cells is above this, both
	 * cells have pressure
	 *
	 ...
	 * be under pressure at equilibrium. Floor order does not matter but ceiling
	 * must be min of both cells.
	 */

...
public class BlockEventList {
	...
	public void addEvent(int x, int y, int z, int amount) {
		eventList.add(new BlockEvent(x, y, z, amount));
	...
	public void processAllEvents() {
		synchronized (this) {
			if (!eventList.isEmpty()) {
				...
				Simulator.SCATTER_GATHER_POOL.completeTask(eventList, Configurator.PERFORMANCE.concurrencyThreshold, e -> {
					if (!(e == null || e.isDeleted())) {
						e.process(maxRetries);
					}
				});
				perfCounter.addCount(eventList.size());
				eventList.removeSomeDeletedItems(EVENT_REMOVAL_PREDICATE);
				perfCounter.endRun();
			}
		}
	}
	...
	public void writeNBT(CompoundTag nbt) {
		...
		final int[] saveData = new int[eventList.size() * BlockEvent.NBT_WIDTH];
		int i = 0;

		for (final BlockEvent event : eventList) {
			if (!event.isDeleted()) {
		...
		nbt.putIntArray(nbtTagName, Arrays.copyOfRange(saveData, 0, i));
	}

	public void readNBT(CompoundTag nbt) {
		eventList.clear();

		final int[] saveData = nbt.getIntArray(nbtTagName);
		...
		if (saveData.length % BlockEvent.NBT_WIDTH != 0) {
			...
			while (i < saveData.length) {
				eventList.add(new BlockEvent(saveData, i));
				i += BlockEvent.NBT_WIDTH;
			}
		}
	}
	...
	public interface BlockEventHandler {
		...
		boolean handleEvent(BlockEvent event);
	}
	...
	public class BlockEvent {
		...
		private void process(int maxRetries) {
			if (eventHandler.handleEvent(this)) {
				...
				retryCount = IS_COMPLETE;
				Pyroclasm.LOG
				.info(String.format("Lava add event @ %1$d %2$d %3$d discarded after max retries. Amount = %4$d", x, y, z, amount));
			}
		}
